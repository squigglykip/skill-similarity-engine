...
Comprehensive diagnostic measures for evaluating the health of the NAB job 
...
Based on: 01_diagnostic_work_prompt.md
...
- Drift and Change Detection (taxonomic integrity over time)

Output: CLI-focused reports with HR-friendly interpretations and actionable insights

Usage:
    python 04_job_architecture_diagnostics.py
"""
...
try:
    ...
    print("⚠️  Note: python-louvain not available, community detection will be skipped")
...
def connect_database():
    ...
    try:
        conn = sqlite3.connect(DiagnosticConfig.DATABASE_PATH)
        print(f"✅ Connected to database: {DiagnosticConfig.DATABASE_PATH}")
        return conn
    except sqlite3.OperationalError as e:
        raise FileNotFoundError(f"Could not connect to database: {e}")

def get_hr_interpretation(silhouette_score: float) -> str:
    """Convert silhouette score to HR-friendly interpretation"""
    if silhouette_score >= DiagnosticConfig.SILHOUETTE_THRESHOLDS['excellent']:
        return "Roles within each area look distinct and well-structured"
    elif silhouette_score >= DiagnosticConfig.SILHOUETTE_THRESHOLDS['good']:
...
def get_action_recommendation(silhouette_score: float, issue_type: str = "general") -> str:
...
def load_diagnostic_data(conn) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
...
def create_job_skill_matrix(job_skills_df: pd.DataFrame) -> pd.DataFrame:
...
def analyze_silhouette_scores(job_skills_df: pd.DataFrame, job_metadata: pd.DataFrame) -> Dict[str, Any]:
    """
    ...
    job_skill_matrix = create_job_skill_matrix(job_skills_df)
    ...
    for func in unique_functions:
        ...
        if len(func_indices) >= DiagnosticConfig.MIN_JOBS_FOR_BU_ANALYSIS:
            func_scores = [job_silhouette_scores[i] for i in func_indices]
            function_silhouettes[func] = {
                'mean': np.mean(func_scores),
                'std': np.std(func_scores),
                'min': np.min(func_scores),
                'job_count': len(func_scores),
                'poor_jobs': sum(1 for score in func_scores if score < DiagnosticConfig.SILHOUETTE_THRESHOLDS['poor'])
            }
    
    # Identify problem jobs
    problem_jobs = []
    for i, score in enumerate(job_silhouette_scores):
        if score < DiagnosticConfig.SILHOUETTE_THRESHOLDS['poor']:
            job_id = job_ids[i]
    ...
    print(f"   → HR Translation: {get_hr_interpretation(overall_silhouette)}")
    print(f"   → Recommendation: {get_action_recommendation(overall_silhouette)}")
    
    if overall_silhouette >= DiagnosticConfig.SILHOUETTE_THRESHOLDS['excellent']:
        print("   🟢 EXCELLENT: Strong functional differentiation across the organization")
    elif overall_silhouette >= DiagnosticConfig.SILHOUETTE_THRESHOLDS['good']:
    ...
    if function_silhouettes:
        ...
        for func, stats in sorted_functions[:5]:
            status = "🟢" if stats['mean'] >= DiagnosticConfig.SILHOUETTE_THRESHOLDS['excellent'] else "🟡" if stats['mean'] >= DiagnosticConfig.SILHOUETTE_THRESHOLDS['good'] else "🔴"
            print(f"   {status} {func}: {stats['mean']:.3f} avg ({stats['job_count']} roles)")
            print(f"      → {get_hr_interpretation(stats['mean'])}")
        
        print(f"\n   Units Needing Attention (Poor Role Differentiation):")
        attention_units = [item for item in sorted_functions if item[1]['mean'] < DiagnosticConfig.SILHOUETTE_THRESHOLDS['good']]
        for func, stats in attention_units[-5:]:
            ...
            print(f"      → {get_action_recommendation(stats['mean'], 'role')}")
    
    print(f"\n⚠️  INDIVIDUAL ROLE ALERTS:")
    if problem_jobs:
        print(f"   → {len(problem_jobs)} roles with poor differentiation (silhouette < {DiagnosticConfig.SILHOUETTE_THRESHOLDS['poor']:.1f})")
        print(f"\n   Most Problematic Roles (requiring immediate review):")
        for job in problem_jobs[:DiagnosticConfig.TOP_N_ISSUES]:
    ...
    return {
        ...
        'job_silhouette_scores': job_silhouette_scores,
...
def analyze_skill_similarity(job_skills_df: pd.DataFrame, job_metadata: pd.DataFrame) -> Dict[str, Any]:
    """
    ...
    for idx, (job_a, job_b) in enumerate(job_pairs):
        ...
        if jaccard_sim > DiagnosticConfig.SIMILARITY_THRESHOLDS['near_duplicate_jaccard']:
        ...
        if cosine_sim > DiagnosticConfig.SIMILARITY_THRESHOLDS['near_duplicate_cosine']:
            job_a_info = job_metadata_indexed.loc[job_a]
            job_b_info = job_metadata_indexed.loc[job_b]
    ...
    for function in job_metadata['JobFunction'].unique():
        ...
        if len(function_jobs) >= DiagnosticConfig.MIN_JOBS_FOR_BU_ANALYSIS:
    ...
    low_variance_functions = [
        ...
        if stats['variance'] < DiagnosticConfig.SIMILARITY_THRESHOLDS['low_variance']
    ]
    ...
    if near_duplicate_jaccard:
        print(f"   🔴 ALERT: {len(near_duplicate_jaccard)} role pairs with high skill overlap (Jaccard > {DiagnosticConfig.SIMILARITY_THRESHOLDS['near_duplicate_jaccard']:.2f})")
        ...
        for pair in sorted_jaccard[:DiagnosticConfig.TOP_N_ISSUES]:
...
def analyze_graph_structure(job_skills_df: pd.DataFrame, job_metadata: pd.DataFrame) -> Dict[str, Any]:
    """
    ...
    skill_degrees = {node: G.degree(node) for node in skill_nodes_list}
    ...
    for skill_node in skill_nodes_list:
        ...
        if degree > 10 and betweenness > DiagnosticConfig.BRIDGE_SKILL_THRESHOLD:
            ...
            for job_node in connected_jobs:
                job_id = int(job_node.replace('job_', ''))
                function = job_metadata[job_metadata['JobProfileID'] == job_id]['JobFunction'].iloc[0]
    ...
    if hub_skills:
        ...
        for skill in hub_skills[:DiagnosticConfig.TOP_N_ISSUES]:
            print(f"   • {skill['skill']}")
...
def analyze_entropy_diversity(job_skills_df: pd.DataFrame, job_metadata: pd.DataFrame) -> Dict[str, Any]:
    """
    ...
    high_entropy_jobs = [job for job in job_entropies if job['entropy'] > DiagnosticConfig.DIVERSITY_THRESHOLDS['high_entropy']]
    low_entropy_jobs = [job for job in job_entropies if job['entropy'] < DiagnosticConfig.DIVERSITY_THRESHOLDS['low_entropy']]
    ...
    if high_entropy_jobs:
        print(f"\n   🔴 UNFOCUSED ROLES (high entropy > {DiagnosticConfig.DIVERSITY_THRESHOLDS['high_entropy']:.1f}):")
        print(f"   HR Translation: These roles try to do too many things—we may need to clarify their purpose")
        for job in high_entropy_jobs[:DiagnosticConfig.TOP_N_ISSUES]:
    ...
    if low_entropy_jobs:
        print(f"\n   🟡 HIGHLY SPECIALIZED ROLES (low entropy < {DiagnosticConfig.DIVERSITY_THRESHOLDS['low_entropy']:.1f}):")
    ...
    if bu_diversity:
        # Sort by skills per job
        sorted_bu = sorted(bu_diversity.items(), key=lambda x: x[1]['skills_per_job'], reverse=True)
...
def main():
    """Main execution function for job architecture diagnostics"""
    ...
    try:
        ...
        job_skills_df, job_metadata, bu_df = load_diagnostic_data(conn)
        
        # Run all diagnostic analyses
        silhouette_results = analyze_silhouette_scores(job_skills_df, job_metadata)
        similarity_results = analyze_skill_similarity(job_skills_df, job_metadata)
        graph_results = analyze_graph_structure(job_skills_df, job_metadata)
        entropy_results = analyze_entropy_diversity(job_skills_df, job_metadata)
        ...
        if silhouette_results['overall_silhouette'] < DiagnosticConfig.SILHOUETTE_THRESHOLDS['poor']:
            overall_health = "ATTENTION NEEDED"
            major_issues.append("Poor functional differentiation across roles")
        elif silhouette_results['overall_silhouette'] < DiagnosticConfig.SILHOUETTE_THRESHOLDS['good']:
