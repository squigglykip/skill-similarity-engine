...
class DynamicSimilarityThresholds:
    ...
    def _calculate_similarity_distribution(self):
        ...
        try:
            ...
            print(f"   • Total similarity pairs: {len(scores):,}")
            print(f"   • Mean similarity: {np.mean(scores):.3f}")
            print(f"   • Median similarity: {np.median(scores):.3f}")
            print(f"   • 95th percentile: {percentile_values[-1]:.3f}")
            print(f"   • 90th percentile: {percentile_values[-2]:.3f}")
            print(f"   • 75th percentile: {percentile_values[-3]:.3f}")
            
        except Exception as e:
            print(f"⚠️ Error calculating similarity distribution: {e}")
            self._similarity_distribution = self._get_default_distribution()
...
class AdaptiveContentSelector:
    ...
    def _get_business_context(self, job_from: str, job_to: Optional[str]) -> Dict:
        ...
        try:
            ...
            print(f"⚠️ Error getting business context: {e}")
