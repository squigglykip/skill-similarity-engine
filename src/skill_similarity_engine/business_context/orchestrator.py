...
from .movement_integrator import MovementIntegrator
...
class BusinessContextOrchestrator:
    ...
    def __init__(self, 
        ...
        Args:
            ...
            data_config_path: Path to data sources configuration file (injected dependency)
        ...
        self.data_config_path = data_config_path  # Injected dependency - no hardcoding
    ...
    def show_workforce_intelligence_menu(self) -> str:
        ...
        Display workforce intelligence database generation menu.
        ...
        print("\n=== Workforce Intelligence Database Generation ===")

        print("Generate comprehensive SQLite database with job similarities and workforce analytics")

        print()

        print("Options:")

        print("1. Generate complete database (recommended)")
    ...
    def handle_workforce_intelligence_menu(self) -> None:
        ...
        """Handle workforce intelligence menu interactions."""
    ...
    def _show_advanced_options(self) -> None:
        """Show advanced database generation options."""
        print("\n=== Advanced Options ===")
    ...
    def _get_database_path(self, interactive: bool = True) -> Optional[Path]:
        ...
        try:
            ...
            db_path = quarter_dir / "workforce_intelligence.sqlite"
    ...
    def _generate_complete_database(self) -> None:
        ...
        try:
            ...
            # Use injected configuration path (dependency injection - no hardcoding)
            if not self.data_config_path:
                print("❌ No data configuration path provided. Cannot load data.")
                return
            
            # Initialize DataLoader with explicit configuration path
            data_loader = DataLoader(str(db_path), config_path=self.data_config_path)
            ...
            if latest_similarity_file:
                ...
                print("⚠️ No similarity matrices found - database will be incomplete")
                print("  Run similarity computation first (main menu option 1)")
            
            # Step 4: Load movement analysis data
            print("\n[4/6] Loading movement analysis data...")
            movement_integrator = MovementIntegrator(str(db_path))
            if not movement_integrator.integrate_movement_data():
                print("⚠️ Movement analysis integration failed - continuing without movement data")
                print("   Note: Run movement analysis precompute first (main.py → 1 → 3)")
            else:
                print("✅ Movement analysis data loaded")
            
            # Step 5: Validate database
            print("\n[5/6] Validating database integrity...")
            validator = DatabaseValidator(str(db_path))
            ...
            # Step 6: Generate statistics and complete
            print("\n[6/6] Generating database statistics...")
            self._collect_final_statistics(db_path)
            
            end_time = datetime.now()
            
            print(f"\n🎉 WORKFORCE INTELLIGENCE DATABASE COMPLETE!")
            print(f"📁 Database location: {db_path}")
            if db_path.exists():
                file_size_mb = db_path.stat().st_size / (1024 * 1024)
                print(f"💾 Database size: {file_size_mb:.1f} MB")
            print(f"⏱️ Total time: {end_time - start_time}")
            print(f"\n🚀 Ready for skills analysis, career pathways, and workforce intelligence!")
