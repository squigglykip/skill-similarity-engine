...
class DataExporter:
    ...
    def export_job_similarity_matrix(
        ...
        if not department_jobs:
            ...
            logger.warning(f"No jobs found for department: {department}")
            return pd.DataFrame()
        
        # Log information about the jobs in this department
        import logging
        logger = logging.getLogger("skill_similarity_engine")
        logger.debug(f"Processing {len(department_jobs)} jobs in department {department}")
        
        # Check if jobs have skills
        jobs_with_skills = [job for job in department_jobs if job.skills]
        logger.debug(f"Department {department} has {len(jobs_with_skills)}/{len(department_jobs)} jobs with skills")
        
        if len(jobs_with_skills) < 2:
            logger.warning(f"Department {department} has fewer than 2 jobs with skills, cannot compute similarities")
            return pd.DataFrame()
        ...
        for job1 in department_jobs:
            if not job1.skills:
                logger.debug(f"Skipping job {job1.job_id} ({job1.title}) as it has no skills")
                continue
        ...
        if not df.empty:
            ...
            logger.debug(f"Generated {len(matrix_data)} similarity pairs for department {department}")
        else:
            logger.warning(f"No similarity data generated for department {department}")
